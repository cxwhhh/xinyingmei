...
/**
 * 利兹大学 (University of Leeds) 匹配算法 - 2025/26 修正版
 * 
 * 核心逻辑变更：
 * 根据用户反馈修正 Tier 划分：
 * 1. Tier 1: 985 + 211 + 双一流 + 强双非 (Leeds认可的非211名单)。
 * 2. Tier 2: 普通公办本科院校。
 * 3. Tier 3: 独立学院 + 民办院校。
 * 
 * 学院要求：
 * 1. Media / Business / CS: 仅接受 Tier 1 & Tier 2 (Tier 3 拒信)。
 * - Tier 1: 75%
 * - Tier 2: 80%
 * 2. Social Sciences / Law: 全 Tier 接受。
 * - Tier 1: 70%
 * - Tier 2: 75%
 * - Tier 3: 80%
 * 3. Engineering / Science / Arts: 全 Tier 接受。
 * - Tier 1: 75%
 * - Tier 2: 80%
 * - Tier 3: 85%
 */
...
public class UKLeedsMatchingAlgorithm implements SchoolMatchingStrategy {

    private static final Logger log = LoggerFactory.getLogger(UKLeedsMatchingAlgorithm.class);
    ...
    // =================================================================================
    // Tier 1: 985 / 211 / 双一流 / 强双非 (Leeds Recognized List)
    // =================================================================================
    ...
    public List<MatchingResult> computeMatchingResults(MatchingRequest request) {
        ...
        if (school == null) {
            ...
            school = ukSchools.stream()
                    .filter(s -> s.getEnglishName() != null
                            && s.getEnglishName().equalsIgnoreCase("University of Leeds"))
                    .findFirst()
                    .orElse(null);
            if (school == null)
                return results;
        ...
        // Tier 3 标记 (用于部分学院拒信判断)
        boolean isTier3 = "Tier 3".equals(studentTier);
        // 是否在名单内 (Tier 1/2/3 都算在内)
        boolean isListed = !"UNKNOWN".equals(studentTier);
        ...
        for (Major major : majors) {
            String facultyCategory = getMajorCategory(major);
            double requiredGpa = getRequiredGpa(studentTier, facultyCategory);

            // 检查拒信逻辑: 热门学院不接受 Tier 3
            if (isTier3 && isTier3Rejected(facultyCategory)) {
                requiredGpa = 999.0;
            }

            double matchScore = calculateMatchScore(studentInfo, requiredGpa, studentTier, facultyCategory, isListed);
            ...
            result.setRegion("英国");
            result.setAlgorithmStrategy("UK_LEEDS_MATCHING_ALGORITHM");

            String analysis = generateRiskAnalysis(studentInfo, studentTier, facultyCategory, requiredGpa, matchScore,
                    isListed);
    ...
    private String getMajorCategory(Major major) {
        ...
        if (name.contains("law") || engName.contains("law") ||
                name.contains("education") || engName.contains("education") ||
                name.contains("politics") || engName.contains("politics") ||
                name.contains("sociology") || engName.contains("sociology") ||
                name.contains("social policy") || engName.contains("social policy") ||
                name.contains("tesol") || engName.contains("tesol")) {
            return "SOCIAL_SCIENCES";
        }
        if (name.contains("media") || engName.contains("media") ||
                name.contains("communication") || engName.contains("communication") ||
                name.contains("journalism") || engName.contains("journalism") ||
                name.contains("film") || engName.contains("film")) {
            return "MEDIA";
        }
        if (name.contains("design") || engName.contains("design") ||
                name.contains("english") || engName.contains("english") ||
                name.contains("history") || engName.contains("history") ||
                name.contains("music") || engName.contains("music")) {
            return "ARTS";
        }
        if (name.contains("business") || engName.contains("business") ||
                name.contains("finance") || engName.contains("finance") ||
                name.contains("marketing") || engName.contains("marketing") ||
                name.contains("management") || engName.contains("management") ||
                name.contains("accounting") || engName.contains("accounting") ||
                name.contains("economics") || engName.contains("economics")) {
            return "BUSINESS";
        }
        if (name.contains("computer science") || engName.contains("computer science")) {
            return "COMPUTER_SCIENCE";
        }
        if (name.contains("chemistry") || engName.contains("chemistry") ||
                name.contains("food") || engName.contains("food")) {
            return "CHEMISTRY_FOOD";
        }
        return "GENERAL";
    }
     ...
     * Tier 3 拒信判断
     */
    private boolean isTier3Rejected(String category) {
        return "MEDIA".equals(category) || "BUSINESS".equals(category) || "COMPUTER_SCIENCE".equals(category);
    }
        ...
        if ("Tier 3".equals(tier)) {
            switch (category) {
                ...
                case "GENERAL":
                    return 85.0;
                default:
                    return 999.0; // Media/Biz/CS Reject Tier 3
            }
        }
        return 999.0;
    }
    ...
    private double calculateMatchScore(MatchingRequest.StudentInfoDTO student, double required, String tier,
            String category, boolean isListed) {
        if (!isListed)
            return 0.0;
        if (required >= 900.0)
            return 0.0;
        ...
        // 热门专业封顶
        if ("MEDIA".equals(category) || "BUSINESS".equals(category)) {
            if (score > 90)
                score = 90;
        } else {
            if (gpa >= required)
    ...
    private String generateRiskAnalysis(MatchingRequest.StudentInfoDTO student, String tier, String category,
            double required, double score, boolean isListed) {
        StringBuilder sb = new StringBuilder();
        sb.append("【利兹大学匹配分析】\n");

        if (!isListed) {
            sb.append("您的本科院校未在利兹大学的认可名单 (Tier 1/2/3) 中，申请风险极高。\n");
            return sb.toString();
        }

        sb.append("1. 院校层级：").append(tier).append(" (Tier 1=985/211/强双非, Tier 2=普通双非, Tier 3=独立/民办)。\n");
        sb.append("2. 申请专业：").append(category).append(" 类。\n");

        if (required >= 900.0) {
            sb.append("风险提示：该类专业不接受 Tier 3 院校申请，建议更换专业或学校。\n");
        } else {
            sb.append("3. 均分要求：最低 ").append((int) required).append("%。\n");
            sb.append("4. 您的均分：").append(student.getGpa()).append("%。\n");
            if (student.getGpa() >= required) {
                sb.append("结果：成绩达标，符合申请要求。");
            } else {
                sb.append("结果：成绩未达标，建议冲刺或Argue。");
            }
        }
        return sb.toString();
    }
